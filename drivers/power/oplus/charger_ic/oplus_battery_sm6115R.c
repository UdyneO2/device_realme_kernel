...
//#include "charger_ic/oplus_battery_sm4250Q.h"
...
#define OPLUS_DETEACH_UNEXPECTLY_INTERVAL round_jiffies_relative(msecs_to_jiffies(110))
#endif
...
#else
	if (chg->hvdcp_disable || chg->qg_ext_sense)
		enable = false;
#endif
...
extern void schgm_flash_torch_priority(struct smb_charger *chg, enum torch_mode mode);
bool oplus_get_mmi_chg_state(void)
{
	bool stat = false;
	...
	stat = g_oplus_chip->mmi_chg == 0 || g_oplus_chip->mmi_fastchg == 0;
	return stat;
}
...
unsuspend:
	...
	if (oplus_chg_get_voocphy_support() && !oplus_get_mmi_chg_state()) {
				...
				const union power_supply_propval *val)
{
	...
	if (val->intval > chg->thermal_levels)
		return -EINVAL;
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (oplus_chg_get_voocphy_support() || chip->qc_9v2a_support) {
		smblib_err(chg, "oplus_chg_get_voocphy_support return\n");
		return 0;
	}
#endif
				    ...
				    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_TYPE_USB_PD:
		if (chg->chg_param.smb_version == PMI632_SUBTYPE){
			if(oplus_get_usb_status() & USB_AUTO_DISCONNECT){
				val->intval = MICRO_7V;
				chg_err("====zk set 7v\n");
			}else{
				val->intval = MICRO_9V;
...
unsuspend_input:
	...
	if (!chg->qc2_unsupported_voltage &&
		...
		if (stat & QC_12V_BIT) {
			...
			//#ifdef OPLUS_FEATURE_CHG_BASIC
			/*zoutao@ODM_HQ.Charge non-QC2.0-compliant chargers force 5v */
			rc = smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_5V_BIT, FORCE_5V_BIT);
			if (rc < 0)
				smblib_err(chg, "qc2_unsupported_voltage failed set to 5V rc=%d\n",
						rc);
			//#endif
		}
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		if (oplus_vooc_get_fastchg_started() == false && g_oplus_chip) {
			...
			smblib_err(chg, "%s:chargerid_volt_got 2 false\n", __func__);
...
extern void oplus_vooc_vooc_monitor_stop(void);
extern int oplus_voocphy_hw_setting_reset(void);
extern int oplus_voocphy_clear_interrupt(void);
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		if (oplus_chg_get_voocphy_support() == true) {
			oplus_voocphy_clear_interrupt();
			set_lpm_disallowed_flag(true);
			//cpuboost_charge_event(CPU_CHG_FREQ_STAT_UP);
			chg_debug("set_lpm_disallowed_flag true %d up cpu freq\n", vbus_rising);
		}
#ifdef OPLUS_FEATURE_CHG_BASIC
		chg->qc_detect_time= cpu_clock(smp_processor_id()) / 1000000;
		if(((chg->qc_detect_time - chg->qc_detach_time) <= 100)
				&& (g_oplus_chip->pre_charger_type == CHARGER_SUBTYPE_QC_2 
					|| g_oplus_chip->pre_charger_type == CHARGER_SUBTYPE_QC_3)){
			chg->qc_break_unexpeactly = true;
			chg->keep_ui = false;
			if(g_oplus_chip->pre_charger_type == CHARGER_SUBTYPE_QC_2){
		...
		if (oplus_chg_get_voocphy_support() == true) {
			if(((chg->qc_detect_time - chg->qc_detach_time) <= 1000)){
				if (g_oplus_chip->voocphy.disconn_pre_charger_type == CHARGER_SUBTYPE_FASTCHG_SVOOC) {
					g_oplus_chip->detect_detach_unexpeactly = 1;
					chg_err("====zk chg->detect_detach_unexpeactly = %d\n",g_oplus_chip->detect_detach_unexpeactly);
				}
			} else {
				opchg_voocphy_clear_cnt_info();
				g_oplus_chip->detect_detach_unexpeactly = 0;
			}
			g_oplus_chip->voocphy.disconn_pre_charger_type = 0;
		}

		g_oplus_chip->pre_charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		if (oplus_chg_get_voocphy_support() == true) {
			...
			chg_debug("set_lpm_disallowed_flag false %d recover cpu freq\n", vbus_rising);
			oplus_vooc_vooc_monitor_stop();
		}
		...
		if( g_oplus_chip->vooc_project == 0){
			chg->keep_ui = true;
			schedule_delayed_work(&chg->qc_detach_unexpectly_work, OPLUS_DETEACH_UNEXPECTLY_INTERVAL);
		}
		if (oplus_chg_get_voocphy_support() == true) {
			g_oplus_chip->voocphy.cpu0_freq = cpufreq_get(0);
			g_oplus_chip->voocphy.cpu1_freq = cpufreq_get(1);
			g_oplus_chip->voocphy.cpu2_freq = cpufreq_get(2);
			g_oplus_chip->voocphy.cpu3_freq = cpufreq_get(3);
			g_oplus_chip->voocphy.disconn_pre_ibat = g_oplus_chip->icharging;
			g_oplus_chip->voocphy.disconn_pre_ibus = g_oplus_chip->ibus;
			g_oplus_chip->voocphy.disconn_pre_vbat = g_oplus_chip->batt_volt;
			g_oplus_chip->voocphy.disconn_pre_vbus = g_oplus_chip->charger_volt;
		}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		//porting from mt6885
		printk(KERN_ERR "[%s]: [%d %d %d %d %d]\n", __func__, vbus_rising, oplus_vooc_get_fastchg_started(),
		oplus_vooc_get_fastchg_to_normal(), oplus_vooc_get_fastchg_to_warm(), oplus_vooc_get_fastchg_dummy_started());
		if (oplus_vooc_get_fastchg_started() == true && oplus_vooc_get_fastchg_to_normal() == false	//plug out by normal
			&& oplus_vooc_get_fastchg_to_warm() == false && oplus_voocphy_user_exit_fastchg() == false
			&& oplus_voocphy_get_btb_temp_over() == false) {
			//oplus_chg_wake_update_work();
			smblib_err(chg, "%s:plug out normal\n", __func__);
			if (oplus_chg_get_voocphy_support()) {
				oplus_voocphy_hw_setting_reset();
			}
			oplus_vooc_reset_fastchg_after_usbout();
			if (g_oplus_chip) {
				...
				g_oplus_chip->chargerid_volt_got = false;
				g_oplus_chip->charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
				smblib_err(chg, "%s:chargerid_volt_got 0 false\n", __func__);
				oplus_chg_wake_update_work();
			}
		} else {
			/* Yichun.Chen	PSW.BSP.CHG  2019-04-08  for charge */
			smblib_err(chg, "%s:plug out fastchg_to_normal or fastchg_to_warm or 5v2a\n", __func__);
			if (oplus_vooc_get_fastchg_started() == false && g_oplus_chip) {
				if (oplus_chg_get_voocphy_support()) {
					oplus_voocphy_hw_setting_reset();
				}
				...
				g_oplus_chip->chargerid_volt = 0;
				g_oplus_chip->chargerid_volt_got = false;
				smblib_err(chg, "%s:chargerid_volt_got 1 false chg->keep_ui = %d\n", __func__, chg->keep_ui);
				g_oplus_chip->charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
				oplus_chg_wake_update_work();
			}
			chg->pre_current_ma = -1;
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static void oplus_qc_detach_unexpectly_work(struct work_struct *work)
{
       struct smb_charger *chg = container_of(work, struct smb_charger,
       ...
       if (chg == NULL)
               return;
	if(chg->qc_break_unexpeactly){
		rc = oplus_get_usb_status();
		if(rc & USB_AUTO_DISCONNECT){
			oplus_clear_usb_status(USB_AUTO_DISCONNECT);
		}
	}
	if(chg->keep_ui){
		chg->keep_ui = false;
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	...
	if((apsd_result->pst == POWER_SUPPLY_TYPE_USB_HVDCP 
		...
		&& g_oplus_chip->pre_charger_type == CHARGER_SUBTYPE_DEFAULT
		&& g_oplus_chip->force_vbus_limit == false){
		if(apsd_result->pst == POWER_SUPPLY_TYPE_USB_HVDCP){
			if(qc2 == false){
				qc2 = true;
			}else{
	...
	chg_err("====zk g_oplus_chip->pre_charger_type = %d qc2 = %d\n",g_oplus_chip->pre_charger_type,qc2);
#endif
...
static void smblib_uusb_otg_work(struct work_struct *work)
{
	...
	if (gpio_is_valid(chg->usb_id_gpio)) {
		if (chg->otg_switch == false)
			...
			otg = !gpio_get_value(chg->usb_id_gpio);
	} else{
	...
	pr_err("otg_switch=%d,otg=%d,otg_present=%d\n",chg->otg_switch,otg,chg->otg_present);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	INIT_DELAYED_WORK(&chg->qc_detach_unexpectly_work, oplus_qc_detach_unexpectly_work);
#endif
...
static int smb5_usbid_config_gpio(struct smb_charger *chg, int enable)
{
	int err = 0;
	struct pinctrl *pinctrl;
	struct pinctrl_state *gpio_state;

	if (!chg || !gpio_is_valid(chg->usb_id_gpio)) {
		pr_err("%s:chg is null or usbid gpio invaild \r\n", __func__);
		return -ENODEV;
	}
	...
	if (enable)
		gpio_state = pinctrl_lookup_state(pinctrl, "usbid_active");
	else
	...
	if (!IS_ERR_OR_NULL(gpio_state)) {
		...
		if (err) {
			pr_err("pinctrl usbid state, err=%d\n", err);
			mutex_unlock(&chg->pinctrl_mutex);
			return -EINVAL;
		}
	}
	else
		pr_err("pinctrl usbid gpio_state err\n");
...
irqreturn_t smb5_usbid_change_handler(int irq, void *data)
{
	struct smb5 *chip = data;
	struct smb_charger *chg = &chip->chg;
	...
	if (chg->usb_id_gpio != 0) {
		if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
			cancel_delayed_work_sync(&chg->uusb_otg_work);
			vote(chg->awake_votable, OTG_DELAY_VOTER, true, 0);
			pr_err("smb5_usbid_change_handler Scheduling OTG work\n");
			schedule_delayed_work(&chg->uusb_otg_work,
					msecs_to_jiffies(chg->otg_delay_ms));
			return IRQ_HANDLED;
		}
...
static void smb5_usbid_irq_register(struct smb5 *chip)
{
	int retval = 0;
	union power_supply_propval ret = {0,};

	smb5_usbid_config_gpio(&chip->chg, 1);
	retval = devm_request_threaded_irq(chip->chg.dev,
		chip->chg.usb_id_irq, NULL, smb5_usbid_change_handler,
		IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT,
		"usbid-change", chip);
	if (retval < 0) {
		pr_err("Unable to request usbid-change irq: %d\n", retval);
		smb5_usbid_config_gpio(&chip->chg, 0);
	} else {
		power_supply_get_property(chip->chg.usb_psy,
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	chg->qg_ext_sense = of_property_read_bool(node,
				"qcom,qg-ext-sns");
		pr_err("qg_ext_sense =%d\n",chg->qg_ext_sense);
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	...
	case POWER_SUPPLY_PROP_USB_STATUS:
		val->intval = oplus_get_usb_status();//chg->usb_status;
		break;
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		...
		if (oplus_vooc_get_fast_chg_type() == 0x34) {
			val->intval = CHARGER_SUBTYPE_FASTCHG_VOOC;
		}
		///pr_err("POWER_SUPPLY_PROP_FAST_CHG_TYPE %d\n", val->intval);
		break;
#endif
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		if(gpio_is_valid(chg->usb_id_gpio)) {
		   if (chg->otg_switch != val->intval) {
			chg->otg_switch = val->intval;
			if (chg->otg_switch == 1){
				smb5_usbid_config_gpio(chg, 1);
			}
			else if(chg->otg_switch == 0){
				smb5_usbid_config_gpio(chg, 0);
			}
		  }
		}else{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		val->intval = (chip->prop_status == POWER_SUPPLY_STATUS_FULL);
#else
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if( g_oplus_chip->vooc_project == 0 && !chg->qg_ext_sense){
		rc = smblib_write(chg, USBIN_OPTIONS_1_CFG_REG,
						0x4c);
	}
	if(chg->hvdcp_disable){
		rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
			HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT, 0);
		if (rc < 0)
			smblib_err(chg, "failed to write USBIN_OPTIONS_1_CFG rc=%d\n",
					rc);
	}
...
#endif
#endif
/*wangdengwen@OPLUS.com move out the CONFIG_HIGH_TEMP_VERSION for clear ADC1 pulse per second*/
#ifdef OPLUS_FEATURE_CHG_BASIC
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static void oplus_chg_qc_temp_to_vchg(void)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
	static int count = 0;

	if (chip->temperature > chip->limits.qc_temp_little_too_hot_decidegc
		|| chip->force_vbus_limit || chip->cool_down_force_5v || chip->flash_led_status) {
		chip->chg_ops->vbus_voltage_write(VBUS_LIMIT_5V);
	} else {
		if (!chip->qc_v3p5_spec_support) {
			if ((chip->charger_volt > 7500 && (chip->icharging * -1) < 2000 && chip->batt_volt > chip->limits.vfloat_sw_set)) {
				chip->chg_ops->vbus_voltage_write(VBUS_LIMIT_5V);
				chip->cv_soc = chip->soc;
			...
			if ((chip->charger_volt > 7500 && chip->soc > chip->cv_soc && chip->batt_volt > chip->limits.vfloat_sw_set) || chip->vbatt_over == true) {
				chip->chg_ops->vbus_voltage_write(VBUS_LIMIT_5V);
			} else if (chip->batt_volt <= chip->limits.vfloat_sw_set && chip->temperature <= chip->limits.qc_temp_little_hot_decidegc
				&& chip->cv_soc > chip->soc) {
				chip->chg_ops->vbus_voltage_write(VBUS_LIMIT_9V);
	...
	chg_err("cv_soc = %d, vfloat_sw_set = %d vbus_limit = %d cool_down = %d flash_led = %d\n",
		chip->cv_soc, chip->limits.vfloat_sw_set, chip->force_vbus_limit, chip->cool_down_force_5v, chip->flash_led_status);
}
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
/*zoutao@ODM_HQ.Charge add usbtemp 2020/06/02*/
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	//usbtemp_volt = opluschg_read_adc(USB_TEMPERATURE1);
...
int oplus_get_batt_btbntc_tmp(void)
{
	int batt_btbntc_vol = 0;
	struct smb_charger *chg = NULL;
	static int batt_btb_vol = USBTEMP_DEFAULT_VOLT_VALUE_MV;
	static int batt_btb_vol_pre = USBTEMP_DEFAULT_VOLT_VALUE_MV;
	int batt_btb_tmp = 25;
	int i = 0;
	int rc = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if (IS_ERR_OR_NULL(chg->iio.batt_btb_temp_chan)) {
		...
		batt_btb_vol = batt_btb_vol_pre;
		goto ntcvolt_get_done;
	}

	rc = iio_read_channel_processed(chg->iio.batt_btb_temp_chan, &batt_btbntc_vol);
	if (rc < 0) {
		chg_err("fail to read usb_temp1 adc rc = %d\n", rc);
		batt_btb_vol = batt_btb_vol_pre;
		goto ntcvolt_get_done;
	}
	if (batt_btbntc_vol <= 0) {
		chg_err("[OPLUS_CHG][%s]:batt_btbntc_vol iio_read_channel_processed  get error\n", __func__);
		batt_btb_vol = batt_btb_vol_pre;
		goto ntcvolt_get_done;
	}

	batt_btbntc_vol = batt_btbntc_vol / 1000;
	if (batt_btbntc_vol > USBTEMP_DEFAULT_VOLT_VALUE_MV) {
		batt_btbntc_vol = USBTEMP_DEFAULT_VOLT_VALUE_MV;
	}

	batt_btb_vol = batt_btbntc_vol;
	batt_btb_vol_pre = batt_btbntc_vol;
ntcvolt_get_done:
	/*map btb temp by usbtemp table*/
	for (i = chip->len_array- 1; i >= 0; i--) {
		if (chip->con_volt[i] >= batt_btb_vol)
			break;
		else if (i == 0)
			break;
	}
	batt_btb_tmp = chip->con_temp[i];
...
static int opluschg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	smblib_get_iio_channel(chg, "batt_btb_temp", &chg->iio.batt_btb_temp_chan);
	...
	pr_err("opluschg_parse_custom_dt 2\n");
	...
	chg->usb_id_gpio = of_get_named_gpio(node, "qcom,usbid-gpio", 0);
	pr_err("qcom,usbid-gpio = gpio%d\n",chg->usb_id_gpio);
	if (gpio_is_valid(chg->usb_id_gpio)) {
		rc = gpio_request(chg->usb_id_gpio, "usbid-gpio");
		if (rc) {
			pr_err("unable to request usbid gpio [%d]\n", chg->usb_id_gpio);
			return -EINVAL;
		}

		chg->usb_id_irq = gpio_to_irq(chg->usb_id_gpio);
	} else {
		pr_err("qcom,usbid-gpio unvalid\n");
	}
...
static int opluschg_get_chargerid(void)
{
	...
	if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.chargerid_adc_default)) {
		chg_debug("chargerid not support return 0\n");
		return 0;
	}

	if ((boot_with_console() == false) && (!IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.chargerid_adc_default))) {
...
static int opluschg_set_vbus(int vbus)
{
	int rc = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
	u8 stat=0;
...
//#ifndef OPLUS_FEATURE_CHG_BASIC
	if(g_oplus_chip->pre_charger_type == CHARGER_SUBTYPE_QC_2 || chg->qg_ext_sense){
		if (vbus == VBUS_LIMIT_5V) {
			...
			if(chg->qc2_unsupported_voltage == QC2_NON_COMPLIANT_9V) {
				smblib_err(chg, "opluschg_set_vbus qc2_unsupported_voltage: not support 9v\n");
				return -1;
			}
			//#endif
...
//#endif
	...
	if (rc < 0) {
		pr_err("opluschg_set_vbus failed rc = %d\n",rc);
		return rc;
	}else{
		pr_err("opluschg_set_vbus limit = %d",vbus);
	}
...
static int opluschg_get_fcc(void)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
...
struct oplus_qc_config {
	...
	int jeita;
	int state;
	int (*handler)(struct oplus_qc_config *config, void *data);
};

struct oplus_qc_config_param {
	int temp;
	int volt;
};

static int oplus_qc_input_current_hanlder(struct oplus_qc_config *config, void *data);

static struct oplus_qc_config oplus_qc_config_data[] = {
	{
		.qc_hv_icl = 364,
		.qc_lv_icl = 364,
		.qc_ledon_icl = 2000,
		...
		.state = -ENOKEY,
		.handler = oplus_qc_input_current_hanlder,
	},
	{
		.qc_hv_icl = 1900,
		.qc_lv_icl = 1140,
	...
	},
	{
		...
		.state = -ENOKEY,
		.handler = oplus_qc_input_current_hanlder,
		//.handler = oplus_qc_ht_fv_handler,
	},
};

static int oplus_qc_check_icl(struct oplus_chg_chip *chip) {
	int size = sizeof(oplus_qc_config_data) / sizeof(struct oplus_qc_config);
	int i;
	struct oplus_qc_config_param param;
	...
	for (i = 0; i < size; i++) {
		if (oplus_qc_config_data[i].handler) {
			oplus_qc_config_data[i].handler(&oplus_qc_config_data[i], (void*)&param);
		}
	}
	return 0;
}

static int oplus_qc_set_icl(int current_ma) {
	int rc = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
#define OPLUS_FV_MV_THRESHOLD 4180
#define OPLUS_HT_FV_THRESHOLD 4130
...
static int oplus_qc_input_current_hanlder(struct oplus_qc_config *config, void *data) {
	struct oplus_qc_config_param *param = (struct oplus_qc_config_param*)data;
	...
	config->state = 0;
	oplus_qc_set_icl(param->volt <= OPLUS_FV_MV_THRESHOLD ? config->qc_hv_icl : config->qc_lv_icl);
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
//		oplus_qc_set_icl(config->qc_ledon_icl);
...
//		oplus_qc_set_icl(param->volt <= OPLUS_FV_MV_THRESHOLD ? config->qc_hv_icl : config->qc_lv_icl);
	...
	if (param->volt >= OPLUS_HT_FV_THRESHOLD && param->temp >= 45) {
		chg_debug("vote disable charge\n");
...
static int opluschg_suspend(void)
{
	...
	struct oplus_chg_chip *chip = g_oplus_chip;
	if (oplus_chg_get_voocphy_support() == true) {
		smblib_masked_write(&chip->pmic_spmi.smb5_chip->chg, 0xA663, 0x1, 1);
		smblib_read(&chip->pmic_spmi.smb5_chip->chg, 0xA663, &val);
		chg_err("read 0xA663 val=%d opluschg_suspend\n", val);
	}
...
static int opluschg_unsuspend(void)
{
	...
	if (oplus_chg_get_voocphy_support() == true) {
		smblib_masked_write(&chip->pmic_spmi.smb5_chip->chg, 0xA663, 0x1, 0);
		smblib_read(&chip->pmic_spmi.smb5_chip->chg, 0xA663, &val);
		chg_err("0xA663 val=%d opluschg_unsuspend\n", val);
	}
...
#endif /* OPLUS_FEATURE_CHG_BASIC */
/*#ifdef OPLUS_FEATURE_CHG_BASIC*/
static int oplus_set_qc_config()
{
	struct oplus_chg_chip *chip = g_oplus_chip;
...
//#ifdef OPLUS_FEATURE_CHG_BASIC
/*zoutao@ODM_HQ.Charge add mid boost 2020/07/24*/
static void opluschg_set_usb_mid_boost(bool boost)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
	int rc;
	...
	if (boost) {
		rc = smblib_write(&chip->pmic_spmi.smb5_chip->chg, 0xa663, 1);
		if (rc < 0) {
			chg_err("Couldn't write usb_mid_boost 1, rc=%d\n", rc);
		}
	} else {
		rc = smblib_write(&chip->pmic_spmi.smb5_chip->chg, 0xa663, 0);
		if (rc < 0) {
			chg_err("Couldn't write usb_mid_boost 0, rc=%d\n", rc);
		}
	}
	chg_debug("write usb_mid_boost %d\n", boost);
}
//#endif


struct oplus_chg_operations opluschg_smb5_ops = {
	.chg_qc_temp_to_vchg	= oplus_chg_qc_temp_to_vchg,
	...
	.get_charger_current		= opluschg_get_fcc,
	.set_aicl_point			= opluschg_set_aicl_point,
	.vbus_voltage_write		= opluschg_set_vbus,
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	...
	chg_debug("smb5_chg_config_init end\n");
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	...
	chg_debug("smb5_init_vbus_regulator && smb5_init_vconn_regulator end\n");
	switch (chg->chg_param.smb_version) {
	case PM8150B_SUBTYPE:
	case PM6150_SUBTYPE:
	case PM7250B_SUBTYPE:
...
#endif
	if (gpio_is_valid(chg->usb_id_gpio)){
        pr_err("pascalh usbid zmlin\n");
		chg->otg_switch = false;
		smb5_usbid_irq_register(chip);
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!gpio_is_valid(chg->usb_id_gpio)){
            pr_err("coco cocob typec zmlin\n");
			opluschg_set_otg_switch(false);
		}
#endif
	pr_err("QPNP SMB5 probed successfully\n");

	return rc;

free_irq:
	chg_debug("free_irq start\n");
	smb5_free_interrupts(chg);
cleanup:
	chg_debug("cleanup start\n");
	smblib_deinit(chg);
